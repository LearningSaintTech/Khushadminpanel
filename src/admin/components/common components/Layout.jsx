...
function NotificationBadge({ count }) {
  ...
  return (
    <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 flex items-center justify-center rounded-full bg-danger text-white text-xs font-medium">
      {count > 99 ? "99+" : count}
    </span>
  );
}
    ...
    <div className="min-h-screen bg-canvas p-3">
      <div className="flex h-[calc(100vh-1.5rem)] gap-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-3 transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]">
          {/* <header className="flex h-14 shrink-0 items-center justify-between gap-3 rounded-2xl border border-border bg-white px-4 pl-14 shadow-sm lg:pl-4">
            <h1 className="truncate text-sm font-semibold text-stone-900">{pageTitle}</h1>
            {!filterSidebar && (
              <Link
                to={notificationsPath}
                className="relative shrink-0 rounded-xl p-2 text-stone-600 transition hover:bg-canvas-muted"
